...
int main(int argc, char **argv) {
  ...
  printf("=== STRESS MATMUL-OS SZ=%d iters=%lu ===\n", SZ, (unsigned long)iters);
  fflush(stdout);
  ...
  tiled_matmul_auto(SZ, SZ, SZ, (elem_t*)A, (elem_t*)B, NULL, (elem_t*)C,
      ...
      NO_ACTIVATION, ACC_SCALE_IDENTITY, 0, false, false, false, false, false, 0, OS);
  ...
  if (cpu_bad != 0) {
    ...
    printf("OS_INVALID (기준 자체가 CPU와 불일치)\n");
    return 2;
  }
  ...
  printf("OS_DONE iters=%lu bad=%lu tiles_per_iter=%lu total_tiles=%lu\n",
