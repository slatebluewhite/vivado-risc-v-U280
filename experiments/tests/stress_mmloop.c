    ...
    for (int A_transpose = 0; A_transpose < 2; A_transpose++)
      for (int B_transpose = 0; B_transpose < 2; B_transpose++)
        for (int dataflow = OUTPUT_STATIONARY; dataflow <= WEIGHT_STATIONARY; dataflow++) {
          if (dataflow == OUTPUT_STATIONARY) test_os(A_transpose, B_transpose);
          else                               test_ws(A_transpose, B_transpose);
        }
    if ((it % 20) == 0) { printf("MMLOOP ok=%lu\n", (unsigned long)it); fflush(stdout); }
  }
  printf("MMLOOP_DONE iters=%lu (모두 통과)\n", (unsigned long)iters);
  fflush(stdout);
  exit(0);
}
