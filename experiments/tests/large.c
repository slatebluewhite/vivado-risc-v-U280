// large.c — BERT-large 형상에 레시피를 그대로 적용 (E275).
...
int main(int argc,char**argv){
  ...
  mark("=== BERT-large 단계 형상별 (블록,Kc) 훑기 (권장 구성, M=128, best-of-5) ===");
  ...
  MM=128;
  struct { int K,N; } SH[]={{1024,1024},{1024,4096},{4096,1024}};
  int BLI[]={4,4,4,8}, BLJ[]={4,6,8,4};
  int KcA[]={8,16,32,0,0};       /* Ktil=64 */
  int KcB[]={16,32,64,0,0};      /* Ktil=256 */
  for(unsigned h=0;h<3;h++){ KK=SH[h].K; NN=SH[h].N;
    int *Kcs = (KK==4096)? KcB : KcA;
    for(int b=0;b<4;b++){ BI=BLI[b]; BJ=BLJ[b];
      if ((NN/16) % BJ) continue;               /* 꼬리 블록 회피 */
      for(int c=0;c<5;c++){ KCv=Kcs[c]; if(!KCv) continue;
