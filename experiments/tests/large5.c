// large5.c — E402: BERT-large 층을 **새 Kc 규칙**(E400)으로 다시 잰다.
//
// E401이 BERT-base 층에서 새 조항("목표 4면 합법 최대 Kc, 단 2청크 이상")을 +6.2%로
// 확인했다. BERT-large에서 바뀌는 단계는 FFN1 [1024x4096] 하나뿐이다:
//     Kc 16 (4청크) -> Kc 32 (2청크)
// QKV·출력은 목표 2라 Kc=32 그대로, FFN2는 이미 합법 최대 Kc=64다.
//
// **가드 자체도 검정한다.** (8,8) Kc=64는 sp=1024로 mem2 한계에 정확히 걸려 합법이고
// 1청크가 된다. 규칙은 "2청크 이상"으로 이걸 배제하는데, 그 배제가 옳은지는 이 트랙에서
// 한 번도 직접 안 쟀다. 스케줄 C로 잰다 — 만약 C가 B보다 빠르면 가드가 틀린 것이다.
//
// 스케줄 (전부 m=2·m=3):
//   A 옛최선 : QKV(8,8)Kc32  출력(8,4)Kc32  FFN1(8,8)Kc16  FFN2(8,8)Kc64
//   B 새규칙 : 같음, FFN1만 Kc32
//   C 가드밖 : 같음, FFN1만 Kc64 (1청크)
//   D 블록대안: 같음, FFN1만 (8,4)Kc32
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <stdarg.h>
...
static int run_sched(int sch,int qkv_mode,int mq,int mo,int m1,int m2,double *t){
  /* E402: sch = 단계별 (블록, Kc) 조합.  A=0 B=1 C=2 D=3 */
  static const int QI_[4] ={8,8,8,8}, QJ_[4] ={8,8,8,8}, QK_[4] ={32,32,32,32};
  static const int OI_[4] ={8,8,8,8}, OJ_[4] ={4,4,4,4}, OK_[4] ={32,32,32,32};
  static const int F1I[4] ={8,8,8,8}, F1J[4] ={8,8,8,4}, F1K[4] ={16,32,64,32};
  static const int F2I[4] ={8,8,8,8}, F2J[4] ={8,8,8,8}, F2K[4] ={64,64,64,64};

  int bad=0; double t0;
  setblk(QI_[sch],QJ_[sch],QK_[sch]);
...
int main(int argc, char **argv){
  ...
  for(int a=0;a<3;a++) grt_flush_ctx(&AC[a]);  /* E378: 3가속기 판 — 문맥 셋 다 플러시 */
  ...
  mark("=== E402: BERT-large 층 — 새 Kc 규칙 확인 + 2청크 가드 검정 ===");
  ...
  struct { const char *nm; int bs,qm,mq,mo,m1,m2; } S[] = {
    { "A 옛최선 FFN1 Kc16      m=2", 0,1,2,2,2,2 },
    { "B 새규칙 FFN1 Kc32      m=2", 1,1,2,2,2,2 },
    { "C 가드밖 FFN1 Kc64(1청크) m=2", 2,1,2,2,2,2 },
    { "D 블록대안 FFN1 (8,4)Kc32 m=2", 3,1,2,2,2,2 },
    { "A 옛최선 FFN1 Kc16      m=3", 0,1,3,3,3,3 },
    { "B 새규칙 FFN1 Kc32      m=3", 1,1,3,3,3,3 },
    { "C 가드밖 FFN1 Kc64(1청크) m=3", 2,1,3,3,3,3 },
    { "D 블록대안 FFN1 (8,4)Kc32 m=3", 3,1,3,3,3,3 },
  };
  double base=0;
