// tune_m2.c — 튜닝 규칙이 m=2에서도 같은가 (E299).
// 규칙(E278)은 전부 m=3에서 유도했는데, hidden >= 1536의 권장은 m=2다(E291).
// 같은 훑기를 m=2 기준으로 채점한다.
...
static void one(void){
  ...
  for(int m=1;m<=3;m++){
    ...
    t[m-1]=b; }
  mark("[%4dx%4d] (%d,%d) Kc=%2d 청크%3d nj=%2d | m2 %9.3f  m3 %9.3f | η2 %5.2f η3 %5.2f | %s",
       KK,NN,BI,BJ,KCv,(KK/16+KCv-1)/KCv,(NN/16+BJ-1)/BJ,
       t[1]*1e3,t[2]*1e3, t[1]/(roof/2),t[2]/(roof/3), bad?"FAIL":"PASS"); }
int main(int argc,char**argv){
  ...
  mark("=== m=2 기준 (블록,Kc) 훑기 (M=128, best-of-5) — m=3 우승자와 비교 ===");
  ...
  MM=128;
  struct { int K,N; } SH[]={{768,768},{768,3072},{3072,768}};
  int BLI[]={4,4,4,8}, BLJ[]={4,6,8,4};
  int KcA[]={6,8,12,16,24};      /* Ktil=48 */
  int KcB[]={16,24,32,48,64};    /* Ktil=192 */
  for(unsigned h=0;h<3;h++){ KK=SH[h].K; NN=SH[h].N;
    int *Kcs = (KK==3072)? KcB : KcA;
    for(int b=0;b<4;b++){ BI=BLI[b]; BJ=BLJ[b];
      if ((NN/16) % BJ) continue;               /* 꼬리 블록 회피 */
      for(int c=0;c<5;c++){ KCv=Kcs[c];
        if (KCv*(BI+BJ) > 512) continue;        /* 스크래치패드 */
        if ((KK/16) % KCv) continue;            /* 청크 딱 떨어지게 */
        one(); } } }
  mark("=== BLK84_DONE ===");
  return 0; }
