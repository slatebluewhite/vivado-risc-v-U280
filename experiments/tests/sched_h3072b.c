// sched_h3072b.c — E299: 규칙의 청크 후퇴 결함을 고친 판.
// 규칙은 스크래치패드 초과 시 청크를 **두 배**로 물러나는데, 그러면 합법인 더 큰 Kc를
// 건너뛴다. [3072x3072] (8,4)는 Ktil=192, Kc<=42 이므로 합법 최대가 32(청크6)인데
// 두 배 후퇴는 2->4(Kc48 불법)->8(Kc24)로 가서 18% 손해를 본다.
//   QKV/출력/FFN1: Kc 24 -> **32** (청크 8 -> 6)
//   FFN2 (4,4) Ktil=768, Kc<=64: Kc 48 -> **64** (청크 16 -> 12)
// 사전 등록: experiments/model/PREREG_E292.txt
// 사전 등록: experiments/model/PREREG_E286.txt  예측 305.9 ms, 1.74배.
// 부수 예측: QKV에서 J분할(S4)이 하나씩 배치(S2)를 이긴다.
...
static int run_sched(int qkv_mode,int mq,int mo,int m1,int m2,double *t){
  int bad=0; double t0;
  setblk(8,4,32);
...
int main(int argc, char **argv){
  ...
  mark("=== hidden3072 층, 청크 후퇴 수정판, 규칙+모델 사전등록 검정 (권장구성 1코어+256비트, 50MHz, best-of-3) ===");
  mark("총 %.0f M MAC = 한 가속기 이론 %.2f ms", macs/1e6, macs/256.0/FREQ*1e3);
  mark("%-34s | %8s %8s %8s %8s | %9s | %s",
       "스케줄","QKV","출력","FFN1","FFN2","합계ms","향상");

  struct { const char *nm; int qm,mq,mo,m1,m2; } S[] = {
    { "S1 가속기 1개",                     1,1,1,1,1 },
    { "S2 E237판: QKV 하나씩, 나머지 m=3",  0,3,3,3,3 },
    { "S3 E238판: QKV순차분할, FFN m=2",    1,3,3,2,2 },
    { "S4 전부 순차 J분할 m=3",             1,3,3,3,3 },
    { "S5 전부 m=2 (QKV 하나씩)",           0,2,2,2,2 },
    { "S6 m=2, QKV 순차 J분할",             1,2,2,2,2 },
  };
  double base=0;
  for(unsigned i=0;i<sizeof(S)/sizeof(S[0]);i++){
    double bt[4]={1e30,1e30,1e30,1e30}, bs=1e30; int bad=0;
    run_sched(S[i].qm,S[i].mq,S[i].mo,S[i].m1,S[i].m2,bt);   /* 웜업 */
    for(int r=0;r<3;r++){ double t[4];
      bad+=run_sched(S[i].qm,S[i].mq,S[i].mo,S[i].m1,S[i].m2,t);
