// stagetune.c — BERT 실제 단계 형상에서 (블록, Kc)를 훑는다 (E273).
...
int main(int argc,char**argv){
  ...
  struct { int I,J,Kc; } C[]={{8,4,12},{8,4,16},{8,4,32},
                              {4,6,12},{4,6,16},{4,6,48},
                              {4,4,12},{4,4,16},{4,4,48}};
  for(unsigned h=0;h<3;h++){ KK=SH[h].K; NN=SH[h].N;
    for(unsigned c=0;c<9;c++){ BI=C[c].I; BJ=C[c].J; KCv=C[c].Kc; one(); } }
  mark("=== BLK84_DONE ===");
  return 0; }
