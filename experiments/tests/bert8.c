// bert8.c — E425: BERT-base 층 seq=256을 **다섯 블록 전수**로 잰다 ((16,4) 포함).
// E424의 감사에서 (16,4) 조항이 검증 고리 밖에 있었음이 드러났다 — pick_blocks가
// 반환하지 않고, 검증기 파서도 그 힌트를 못 읽고, measured.py에 (16,4) 실측이 0개다.
// 파서와 힌트는 고쳤고, 이제 표를 채워 조항을 채점할 수 있게 한다.
// 블록별 규칙 Kc: QKV·출력·FFN1은 전부 24, FFN2는 (4,4)96 (8,4)/(4,8)/(8,8)64 (16,4)48.
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <stdarg.h>
...
static int run_sched(int blkset,int qkv_mode,int mq,int mo,int m1,int m2,double *t){
  ...
  /* E381: blkset 0..3 = (4,4) (8,4) (4,8) (8,8) 를 전 단계 균일 적용 */
  /* E425: blkset 0..4 = (4,4)(8,4)(4,8)(8,8)(16,4) 전 단계 균일 */
  static const int BI5[5]={4,8,4,8,16}, BJ5[5]={4,4,8,8,4};
  static const int F2KC5[5]={96,64,64,64,48};
  const int QI=BI5[blkset], QJ=BJ5[blkset];
  const int F1I=BI5[blkset], F1J=BJ5[blkset];
  const int F2I=BI5[blkset], F2J=BJ5[blkset], F2KC=F2KC5[blkset];
  int bad=0; double t0;
  setblk(QI,QJ,24);
...
int main(int argc, char **argv){
  g=fopen(argc>1?argv[1]:"/mnt2/tmp/bert8.log","w");
  ...
  mark("=== E425: BERT-base 층 seq=256 — 다섯 블록 전수 (3가속기+mem2) ===");
  mark("네 블록을 전 단계 균일 적용. Kc는 QKV/출력 24, FFN1 12, FFN2 48.");
  ...
  struct { const char *nm; int bs,qm,mq,mo,m1,m2; } S[] = {
    { "(4, 4)  m=2", 0,1,2,2,2,2 }, { "(4, 4)  m=3", 0,1,3,3,3,3 },
    { "(8, 4)  m=2", 1,1,2,2,2,2 }, { "(8, 4)  m=3", 1,1,3,3,3,3 },
    { "(4, 8)  m=2", 2,1,2,2,2,2 }, { "(4, 8)  m=3", 2,1,3,3,3,3 },
    { "(8, 8)  m=2", 3,1,2,2,2,2 }, { "(8, 8)  m=3", 3,1,3,3,3,3 },
    { "(16, 4) m=2", 4,1,2,2,2,2 }, { "(16, 4) m=3", 4,1,3,3,3,3 },
  };
  double base=0;
