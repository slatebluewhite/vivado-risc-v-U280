// msel.c — 코스트 모델이 "가속기 몇 개에게 나눌지"를 고를 수 있는가 (E240).
//
// 사전 등록(experiments/model/PREREG_E240.txt): 모델은 여섯 형상 **전부** m=3이
// 최적이라고 예측했고, m3/m2 비를 0.857~0.917로 냈다. 실측에서 m=2가 이기는 형상이
// 하나라도 나오면 그 예측은 반증된다.
//
// J 분할만 잰다(E238에서 독립 복제보다 항상 낫다고 확인). 블록 (4,6), Kc=16,
// 블록 단위 교대 발행(E156/E237), best-of-3, 정확성 검사 포함.
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <stdarg.h>
...
int main(int argc,char**argv){
  g=fopen(argc>1?argv[1]:"/mnt2/tmp/msel.log","w");
  ...
  mark("=== 모델이 m을 고를 수 있는가: 사전 등록 대 실측 (3i8f50, best-of-3) ===");
  mark("모델 예측: 여섯 형상 전부 m=3 최적, m3/m2 = 0.857~0.917");
  mark("%-14s %7s | %8s %8s %8s | %5s %6s | %s","형상","ws MB","m=1","m=2","m=3","최적","m3/m2","정확성");
  struct { int K,N; } S[] = {{256,768},{1536,768},{768,1536},{1536,1536},{512,2048},{2048,512}};
  for(unsigned i=0;i<sizeof(S)/sizeof(S[0]);i++){
    ...
    for(int m=1;m<=3;m++){
      ...
      t[m-1]=b; }
    int bm=0; for(int m=1;m<3;m++) if(t[m]<t[bm]) bm=m;
    double ws=((double)M*KK+(double)KK*NN+(double)M*NN)/1e6;
    mark("[%4dx%4d]   %6.2fMB | %8.3f %8.3f %8.3f | %5d %6.3f | %s",
         KK,NN,ws,t[0]*1e3,t[1]*1e3,t[2]*1e3,bm+1,t[2]/t[1],bad?"FAIL":"PASS");
