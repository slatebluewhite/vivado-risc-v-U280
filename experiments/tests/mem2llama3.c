...
int main(int argc,char**argv){
  ...
  mark("=== E379: Llama FFN, 3가속기+mem2 판 (hidden %d, inter %d), M=%d, best-of-5, 62.5MHz ===",H,FF,MM);
  fill();
  ...
  mark("");
  mark("--- 블록 집합 x 가속기 수 ---");
  mark("A 옛규칙: gate/up (8,4)Kc16, down (4,4)Kc44   (표준 한계에서의 규칙)");
  mark("B 새규칙: 전부 (8,8)                          (mem2 한계에서의 규칙 1순위)");
  mark("C 차점  : 전부 (4,8)                          (도구가 지목한 차점, E369/E371)");
  mark("%-22s | %9s %9s %9s | %9s","블록/배분","gate","up","down","합계ms");
  for(int bs=0; bs<3; bs++){
    int GI = (bs==0)?8 : (bs==1)?8 : 4;
    int GJ = (bs==0)?4 : 8;
    int DI = (bs==0)?4 : (bs==1)?8 : 4;
    int DJ = (bs==0)?4 : 8;
    const char *nm = (bs==0)?"A 옛규칙":(bs==1)?"B 새규칙(8,8)":"C 차점(4,8)";
    for(int m=1;m<=3;m++){
      ...
      mark("%-14s m=%d | %9.3f %9.3f %9.3f | %9.3f  %s",
           nm,m,bg*1e3,bu*1e3,bd*1e3,(bg+bu+bd)*1e3, chk_g()?"FAIL":"PASS");
    }
  }
  mark("=== E379_DONE ===");
  return 0; }
