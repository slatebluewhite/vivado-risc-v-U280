// nsweep.c — E310: 이중모드가 N의 날카로운 함수인가.
// 사전 등록: experiments/model/PREREG_E310.txt
//
// E307: 이중모드는 스케줄링이 아니고 재현된다. 형상별로 묶으니 N=1024·1280만
// 이중모드고 N=1152·1536·2048은 0/9칸이다. 게다가 그 형상들은 m=1에서도 시끄럽다.
// K=768 고정, (4,4) 고정, N을 64씩 훑어 1024·1280을 감싼다. 프로세스 8회는 밖에서.
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <stdarg.h>
...
int main(int argc,char**argv){
  g=fopen(argc>1?argv[1]:"/mnt2/tmp/nsweep.log","w");
  ...
  mark("=== E310 N 훑기: 이중모드가 N의 날카로운 함수인가 (K=768, (4,4), M=128) ===");
  mark("%11s %6s %4s %5s | %9s %9s %9s | %11s | %s",
       "형상","블록","Kc","청크","m=1","m=2","m=3","η(2) η(3)","정확성");
  MM=128; KK=768; BI=4; BJ=4;
  int NS[]={896,960,1024,1088,1152,1216,1280,1344};
  int CH[]={2,4};
  for(unsigned h=0;h<8;h++){ NN=NS[h];
    for(int c=0;c<2;c++){ KCv=(KK/16)/CH[c]; one(); } }
  mark("=== NSWEEP_DONE ===");
  return 0; }
