// kcm.c — E413: M=256에서 새로 합법이 되는 (16,4)이 강도 규칙을 깨는가.
// 사전 등록: PREREG_E413.txt.  tune.py의 all_blocks()는 I를 8에서 자르는데 그건
// M=128(TI=8) 시절 제약이다. M>=256이면 (16,4)가 I*J=64<=64로 합법이고 강도 5.0으로
// (8,8)의 4.0과 (8,4)의 6.0 사이에 있다 — 이 트랙이 한 번도 안 잰 지점이다.
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <stdarg.h>
...
int main(int argc,char**argv){
  g=fopen(argc>1?argv[1]:"/mnt2/tmp/kcm.log","w");
  ...
  mark("=== E413: M=256, (16,4)을 포함한 블록 비교 (규칙 Kc, best-of-3) ===");
