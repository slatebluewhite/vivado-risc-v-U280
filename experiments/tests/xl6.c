// xl6.c — E410: h3072 층 블록 전수를 **새 Kc 규칙**(E406/E408)으로 다시 훑는다.
// measured.py의 표는 옛 Kc로 잰 것이라 도구가 권하는 조합을 채점할 수 없었다(E407 주석).
// 규칙이 이 워크로드에 주는 (단계, 블록)별 Kc:
//   QKV·출력 Kc=48   FFN1 Kc=48   FFN2 (4,4)Kc=128 나머지 Kc=64
...
static int run_sched(int blkset,int qkv_mode,int mq,int mo,int m1,int m2,double *t){
  ...
  static const int F2KCr[4]={128,64,64,64};   /* E410: FFN2는 블록마다 다름 */
...
int main(int argc, char **argv){
  g=fopen(argc>1?argv[1]:"/mnt2/tmp/xl6.log","w");
  ...
  mark("=== E410: h3072 블록 전수 — 새 Kc 규칙 (3가속기+mem2) ===");
