...
static unsigned long FAILCNT = 0;   // 실패해도 계속 진행하며 센다
...
int main(int argc, char **argv) {
  size_t iters = argc > 1 ? (size_t)atoi(argv[1]) : 200;
#ifndef BAREMETAL
  ...
  // matmul.c 본체를 한 프로세스 안에서 반복한다.
  // test_os/test_ws는 실패 시 exit(1)로 끝나므로, 여기까지 도달하면 그 회차는 통과다.
    ...
    for (int A_transpose = 0; A_transpose < 2; A_transpose++)
      for (int B_transpose = 0; B_transpose < 2; B_transpose++)
        for (int dataflow = OUTPUT_STATIONARY; dataflow <= WEIGHT_STATIONARY; dataflow++) {
          if (dataflow == OUTPUT_STATIONARY) test_os(A_transpose, B_transpose);  // OS만
        }
    if ((it % 100) == 0) { printf("OSCOUNT prog=%lu fails=%lu\n", (unsigned long)it, FAILCNT); fflush(stdout); }
  }
  printf("OSCOUNT iters=%lu fails=%lu\n", (unsigned long)iters, FAILCNT);
  fflush(stdout);
  exit(0);
}
