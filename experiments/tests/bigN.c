// bigN.c — (8,4)에서 b(N)은 언제 살아나는가 (E263).
...
int main(int argc,char**argv){
  ...
  mark("=== (8,4)에서 b(N)의 출현 지점 (3i8f50 현재 상태, M=128, Kc=16, best-of-5) ===");
  mark("a(K): 512:0.36  2048:1.26.  K=512 행이 b>0.36에서 오르기 시작해야 한다");
  mark("%4s %11s %5s %9s | %9s %9s %9s | %17s | %s",
       "M","형상","nj","루프라인","m=1","m=2","m=3","η(1) η(2) η(3)","정확성");
  int Ks[]={512,2048}, Ns[]={2304,3072,4608,6144};
  MM=128;
  for(int a=0;a<2;a++) for(int b=0;b<4;b++){ KK=Ks[a]; NN=Ns[b]; one(); }
  mark("=== BLK84_DONE ===");
  return 0; }
