...
#define MMAX 128
#define ASTR_MAX 4160   /* E373: A 행 스트라이드 패딩 */
#define KMAX 4096
#define NMAXV 2048
static int NN_ = 1024;   /* E368: 런타임 인자 */
#define FREQ 62.5e6
static int BI=4, BJ=4, KCv=64;
static int8_t A[MMAX*ASTR_MAX]    __attribute__((aligned(64)));
static int ASTR;   /* A의 행 스트라이드(원소). 기본 = KK */
...
static void it_step(int a, iter *s){
  ...
  grt_block_ksplit(&AC[a], I,J,KK/16,KCv,
                   A+(size_t)s->i0*16*ASTR, B+(size_t)j0*16,
                   C+(size_t)s->i0*16*NN_+(size_t)j0*16, ASTR,NN_,NN_, 1);
  s->i0 += BI;
  if(s->i0>=TI){ s->i0=0; s->jb+=s->step; if(s->jb*BJ>=Ntil) s->done=1; } }
static void run(int m){
  iter s[NACC];
  for(int a=0;a<m;a++){ grt_loop_ws_config(&AC[a], ASTR,NN_,NN_);
...
static void fill(void){
  memset(A,0,(size_t)MM*ASTR);
  for(int r=0;r<MM;r++){ int p=r%(KK-1); A[(size_t)r*ASTR+p]=1; A[(size_t)r*ASTR+p+1]=1; }
...
int main(int argc,char**argv){
  ...
  mark("=== E373: A 행 스트라이드 패딩 검정 (N=%d, M=128, mem2, best-of-5) ===", NN_);
  mark("각 K를 스트라이드=K(기본)와 스트라이드=K+64(패딩) 두 번 잰다.");
  ...
  const int KS[2]={3584,4096};
  const int CH[4]={2,4,6,8};
  struct { int I,J; } BL[4]={{4,4},{8,4},{4,8},{8,8}};
  for(int k=0;k<2;k++){
   for(int pad=0; pad<2; pad++){
    KK=KS[k]; int Ktil=KK/16; ASTR = KK + (pad?64:0);
    mark("--- K=%d (Ktil=%d) 스트라이드 %d %s ---",KK,Ktil,ASTR,pad?"(패딩)":"(기본)");
    fill();
    for(int b=0;b<4;b++){
      BI=BL[b].I; BJ=BL[b].J;
