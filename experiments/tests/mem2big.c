...
static int run_sched(int blkset,int qkv_mode,int mq,int mo,int m1,int m2,double *t){
  /* E361: 0=A(옛 한계의 규칙) 1=B(새 규칙, 전부 (8,8)) 2=C(FFN2만 (8,8)) */
  const int QJ  = blkset==1 ? 8 : 4;
  const int F1J = blkset==1 ? 8 : 4;
...
int main(int argc, char **argv){
  ...
  mark("=== E361: mem2 판 hidden2048 층 — 옛 규칙 대 새 규칙, 62.5MHz, best-of-3 ===");
  mark("A=(8,4)x3/(4,4) | B=전부 (8,8) | C=FFN2만 (8,8).  FFN2=[8192x2048], 합성에서 +24.2%%였다");
