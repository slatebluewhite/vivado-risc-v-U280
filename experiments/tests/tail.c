...
static int8_t B[2048*2048]  __attribute__((aligned(64)));
static int8_t C[M*2048]     __attribute__((aligned(64)));
static int8_t REF[M*2048];
...
int main(int argc,char**argv){
  ...
  mark("사전 등록: BJ=4(정확)로 바꾸면 모델 오차가 ~1.3%로 떨어지고 6/6이 된다");
  mark("%11s %6s %6s %5s | %8s %8s %8s | %s","형상","(I,J)","블록수","꼬리","m=1","m=2","m=3","정확성");
  struct { int K,N; } S[]={{512,2048},{2048,512}};
  int BIs[]={4,4}, BJs[]={6,4};
  for(int si=0;si<2;si++){
    KK=S[si].K; NN=S[si].N; fill();
    for(int bi=0;bi<2;bi++){
      gBI=BIs[bi]; gBJ=BJs[bi];
