...
static int run_sched(int blkset,int qkv_mode,int mq,int mo,int m1,int m2,double *t){
  ...
  const int QI = blkset==1 ? 8:8, QJ = blkset==1 ? 8:4;
  const int F1I= blkset==1 ? 8:8, F1J= blkset==1 ? 8:4;
  const int F2I= blkset==0 ? 4:8, F2J= blkset==0 ? 4:8;
  int bad=0; double t0;
  setblk(QI,QJ,24);
...
int main(int argc, char **argv){
  ...
  mark("=== E361: mem2 판(sp 2배, acc 2배) BERT 층 — 옛 규칙 대 새 규칙, 62.5MHz, best-of-3 ===");
  mark("A=(8,4)/(8,4)/(4,4) 강도 6.0/6.0/8.0  |  B=전부 (8,8) 강도 4.0  |  C=FFN2만 (8,8)");
