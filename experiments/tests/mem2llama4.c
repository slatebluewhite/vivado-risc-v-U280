...
static void mm_jsplit(int m,const int8_t*A,const int8_t*W,int8_t*C,int K,int N){
  ...
  int i0[3]={0,0,0}, jb[3], done[3];   /* E391: m=3 대비 (E379의 버그) */
...
/* E391: 원본에 chk_y가 있는데 mark에서 호출하지 않고 있었다 — m=3 경로가 미검증이었다. */
...
int main(int argc,char**argv){
  g=fopen(argc>1?argv[1]:"/mnt2/tmp/m2_llama.log","w");
  if(!g){perror("로그");exit(1);}
  if(mlockall(MCL_CURRENT|MCL_FUTURE)!=0) mark("mlockall 실패(계속)");
  cpu_set_t cs; CPU_ZERO(&cs); CPU_SET(0,&cs); sched_setaffinity(0,sizeof(cs),&cs); sched_yield();
  for(int a=0;a<3;a++) grt_flush_ctx(&AC[a]);  /* E379: 3가속기 판 */
  mark("=== E380: Llama FFN 블록 전수 (gate/up Kc=16, down Kc=44), 3가속기+mem2 판 (hidden %d, inter %d), M=%d, best-of-5, 62.5MHz ===",H,FF,MM);
  fill();
  ...
  mark("");
  mark("--- 블록 집합 x 가속기 수 ---");
  mark("네 블록을 단계마다 균일 적용하고 단계별 시간을 남긴다 — 오프라인에서 단계별 조합을 채점한다.");
  
  
  mark("%-22s | %9s %9s %9s | %9s","블록/배분","gate","up","down","합계ms");
  const int BI4[4]={4,8,4,8}, BJ4[4]={4,4,8,8};
  for(int bs=0; bs<4; bs++){
    int GI=BI4[bs], GJ=BJ4[bs], DI=BI4[bs], DJ=BJ4[bs];
    static char nmbuf[4][16]; snprintf(nmbuf[bs],sizeof nmbuf[bs],"(%d,%d)",GI,GJ);
    const char *nm = nmbuf[bs];
    for(int m=1;m<=3;m++){
      ...
      if(m==1) memcpy(Ry,Y,(size_t)MM*H); }   /* E391: m=1 결과를 기준으로 */
      setblk(GI,GJ,16); mm_jsplit(1,X,Wg,Hg,H,FF); grt_fence();
      mark("%-14s m=%d | %9.3f %9.3f %9.3f | %9.3f  %s",
           nm,m,bg*1e3,bu*1e3,bd*1e3,(bg+bu+bd)*1e3,
           (chk_g()||chk_y())?"FAIL":"PASS");
    }
  }
  mark("=== E379_DONE ===");
  return 0; }
