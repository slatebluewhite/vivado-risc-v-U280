// mem2bert.c — E361: 온칩 메모리 2배 판에서 BERT 층. 옛 규칙 대 새 규칙을 같은 이진에서.
//
// mem2 판은 sp <= 1024 타일, I*J <= 64다. 규칙을 새 한계로 돌리면 세 단계 전부 (8,8)을
// 고른다 — 강도가 6.0/6.0/8.0에서 **4.0/4.0/4.0**으로 떨어진다 (FFN2는 바이트 절반).
//
// 블록 집합 A(옛 한계에서의 규칙) : [768x768] (8,4)Kc24 | FFN1 (8,4)Kc12 | FFN2 (4,4)Kc48
// 블록 집합 B(새 한계에서의 규칙) : 전부 (8,8), Kc 24 / 12 / 48
// 블록 집합 C(FFN2만 교체)        : A와 같되 FFN2만 (8,8)Kc48  -> 이득이 어느 단계에서 오는지
//
// 가속기 2개 판이므로 문맥 2(custom1)는 건드리지 않는다 (E337).
// E273 훑기의 단계별 우승자:
...
static int run_sched(int blkset,int qkv_mode,int mq,int mo,int m1,int m2,double *t){
  /* blkset: 0=A(옛 규칙) 1=B(새 규칙, 전부 (8,8)) 2=C(FFN2만 (8,8)) */
  /* E401: blkset = 단계별 (블록, Kc) 조합.
       0 = A 옛 최선 : (8,4)Kc24 (8,4)Kc24 (8,4)Kc12 (8,8)Kc48
       1 = B 새 규칙 : (8,4)Kc24 (8,4)Kc24 (8,8)Kc24 (8,8)Kc64
       2 = C 전부(8,8): (8,8)Kc24 (8,8)Kc24 (8,8)Kc24 (8,8)Kc64  */
  static const int QIs[3]={8,8,8}, QJs[3]={4,4,8}, QKs[3]={24,24,24};
  static const int F1I_[3]={8,8,8}, F1J_[3]={4,8,8}, F1K_[3]={12,24,24};
  static const int F2I_[3]={8,8,8}, F2J_[3]={8,8,8}, F2K_[3]={48,64,64};
  const int QI=QIs[blkset], QJ=QJs[blkset], QK=QKs[blkset];
  const int F1I=F1I_[blkset], F1J=F1J_[blkset], F1K=F1K_[blkset];
  const int F2I=F2I_[blkset], F2J=F2J_[blkset], F2K=F2K_[blkset];
  int bad=0; double t0;
  setblk(QI,QJ,QK);
...
int main(int argc, char **argv){
  ...
  mark("=== E401: BERT-base 층, 새 Kc 확인 (3가속기+mem2) ===");
  mark("A=옛최선(FFN1 (8,4)Kc12, FFN2 (8,8)Kc48) | B=새규칙(FFN1 (8,8)Kc24, FFN2 (8,8)Kc64) | C=전부(8,8)");
  ...
  struct { const char *nm; int bs,qm,mq,mo,m1,m2; } S[] = {
    { "A 옛최선 m=2", 0,1,2,2,2,2 },
    { "A 옛최선 m=3", 0,1,3,3,3,3 },
    { "B 새규칙 m=2", 1,1,2,2,2,2 },
    { "B 새규칙 m=3", 1,1,3,3,3,3 },
    { "C 전부(8,8) m=2", 2,1,2,2,2,2 },
    { "C 전부(8,8) m=3", 2,1,3,3,3,3 },
  };
  double base=0;
