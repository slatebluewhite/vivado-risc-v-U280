// chunkK.c — a는 청크 수만의 함수인가 (청크,K) 둘 다인가 (E272).
...
static void one(void){
  ...
  for(int m=1;m<=3;m++){
    ...
    t[m-1]=b; }
  mark("K=%4d Kc=%2d 청크%3d 타일%4d | %9.3f %9.3f %9.3f | %5.2f %5.2f %5.2f | %s",
       KK,KCv,(KK/16+KCv-1)/KCv,KCv*(BI+BJ),t[0]*1e3,t[1]*1e3,t[2]*1e3,
...
int main(int argc,char**argv){
  ...
  mark("=== 청크 수 대 K (권장 구성 256비트, M=128, 블록 (4,4), N=1152, best-of-5) ===");
  mark("같은 청크 수에서 K를 바꾼다. 일치하면 a=f(청크), 커지면 a=f(청크,K)");
  mark("%5s %11s %4s %7s %9s | %9s %9s %9s | %11s | %s",
       "Kc","형상","청크","루프라인","","m=1","m=2","m=3","η(2) η(3)","정확성");
  MM=128; NN=1152; BI=4; BJ=4;
  struct { int K,Kc; } S[]={{1024,8},{1024,16},{1024,32},
                            {2048,16},{2048,32},{2048,64},
                            {3072,32},{3072,48},{3072,16}};
  for(unsigned i=0;i<sizeof(S)/sizeof(S[0]);i++){ KK=S[i].K; KCv=S[i].Kc; one(); }
  mark("=== BLK84_DONE ===");
  return 0; }
