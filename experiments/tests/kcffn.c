...
int main(int argc,char**argv){
  ...
  mark("=== E397: FFN2 네 개에서 (8,8)의 Kc 전수 (3가속기+mem2, best-of-4) ===");
  mark("%13s %5s %6s %5s %6s | %9s %9s %9s | %5s | %s",
       "형상","블록","Kc","청크","sp","m=1","m=2","m=3","η(3)","정확성");

  /* E397: 비중이 가장 큰 FFN2 네 개에서 (8,8)의 합법 Kc를 전수로 훑는다.
     (8,8)은 Kc*16 <= 1024 이므로 Kc <= 64. 규칙이 고르는 값에 별표를 단다. */
  struct { int K,N,rule; int kcs[5]; } S[4] = {
    { 3072,  768, 48, {48,32,24,16,0} },   /* BERT-base FFN2 */
    { 4096, 1024, 64, {64,32,16,0,0} },    /* BERT-large FFN2 */
    { 6144, 1536, 64, {64,48,32,24,0} },   /* h1536 FFN2 */
    { 8192, 2048, 64, {64,32,16,0,0} },    /* h2048 FFN2 */
  };
  BI=8; BJ=8;
  for(int i=0;i<4;i++){
    KK=S[i].K; NN=S[i].N; fill();
    mark("--- [%dx%d] (8,8), 규칙 Kc=%d ---",KK,NN,S[i].rule);
