...
int main(int argc,char**argv){
  ...
  mark("=== E398: 규칙 Kc 대 합법 최대 Kc (3가속기+mem2, best-of-4) ===");
  mark("%13s %5s %6s %5s %6s | %9s %9s %9s | %5s | %s",
       "형상","블록","Kc","청크","sp","m=1","m=2","m=3","η(3)","정확성");

  /* E398: 규칙의 Kc와 **합법 최대 Kc**가 다른 세 자리를 잰다.
     E299는 옛 하드웨어에서 "합법 최대 Kc"를 기각했다(2.8% 대 1.1%). 새 판에서 다시 본다. */
  struct { int K,N,rule,mx; } S[3] = {
    { 3072,  768, 48, 64 },   /* BERT-base FFN2 : 규칙 48(4청크) 대 최대 64(3청크) */
    {  768,  768, 24, 48 },   /* BERT-base QKV  : 규칙 24(2청크) 대 최대 48(1청크) */
    { 1024, 1024, 32, 64 },   /* BERT-large QKV : 규칙 32(2청크) 대 최대 64(1청크) */
  };
  BI=8; BJ=8;
  for(int i=0;i<3;i++){
    KK=S[i].K; NN=S[i].N; fill();
    mark("--- [%dx%d] (8,8): 규칙 Kc=%d 대 합법 최대 Kc=%d ---",KK,NN,S[i].rule,S[i].mx);
    KCv=S[i].rule; one();
    KCv=S[i].mx;   one();
  }
  mark("=== E398_DONE ===");
