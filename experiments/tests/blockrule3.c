// blockrule3.c — E318: E317 규칙("청크 4 이하 도달 가능")의 자체 표본 밖 검정.
// 사전 등록: experiments/model/PREREG_E318.txt
// 갈리는 셋 [1536x896] [1792x768] [2048x640]: E316은 (4,4), E317은 (8,4)를 지목.
// 대조군 [3072x1536](둘 다 (4,4)), [2048x768](둘 다 (8,4)).
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <stdarg.h>
...
int main(int argc,char**argv){
  g=fopen(argc>1?argv[1]:"/mnt2/tmp/blockrule3.log","w");
  ...
  mark("=== E318 E317 규칙 표본 밖 검정 (M=128, best-of-5) ===");
  mark("%11s %6s %4s %5s | %9s %9s %9s | %11s | %s",
       "형상","블록","Kc","청크","m=1","m=2","m=3","η(2) η(3)","정확성");
  MM=128;
  struct { int K,N; } SH[]={{1536,896},{1792,768},{2048,640},{3072,1536},{2048,768}};
  int BLI[]={8,4,4}, BLJ[]={4,4,8};
  int CH[]={2,3,4,6,8};
  for(unsigned h=0;h<5;h++){ KK=SH[h].K; NN=SH[h].N;
    int Ktil=KK/16;
    mark("--- [%dx%d] K·N=%.2fM ---",KK,NN,(double)KK*NN/1e6);
