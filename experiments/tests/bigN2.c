// bigN2.c — b(N)을 N=12288까지 직접 잰다 (E287).
// E286: b(8192)를 0.50으로 외삽했는데 역산값이 0.96이었다. 두 점(4608·6144)을 포화로 읽은 탓.
// K를 작게 잡아(a가 작다) b를 분리한다. K=512(a≈0.2), K=1024(a≈0.25).
...
int main(int argc,char**argv){
  ...
  mark("=== b(N)을 12288까지 (권장 구성, M=128, 블록 (8,4), best-of-5) ===");
  mark("기존 b표: 3072:0.22 4608:0.54 6144:0.51 — 8192의 역산값이 0.96이었다");
  mark("%5s %11s %4s %7s %9s | %9s %9s %9s | %11s | %s",
       "Kc","형상","청크","루프라인","","m=1","m=2","m=3","η(2) η(3)","정확성");
  int Ks[]={512,1024};
  int Ns[]={3072,4608,6144,8192,12288};
  int CH[]={2,4};
  MM=128; BI=8; BJ=4;
  for(int ki=0;ki<2;ki++){ KK=Ks[ki];
    int Ktil=KK/16;
    for(int ni=0;ni<5;ni++){ NN=Ns[ni];
