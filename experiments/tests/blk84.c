// blk84.c — 최적 블록 (8,4)에서 곡선을 다시 재고 구조 결론을 재확인한다 (E261).
//
...
int main(int argc,char**argv){
  ...
  mark("=== 블록 (8,4)에서 곡선 재측정 (3i8f50 현재 상태, Kc=16, best-of-5) ===");
  mark("%4s %11s %5s %9s | %9s %9s %9s | %17s | %s",
       "M","형상","nj","루프라인","m=1","m=2","m=3","η(1) η(2) η(3)","정확성");
  int Ks[]={512,1024,2048,3072}, Ns[]={768,1152,2304};
  MM=128;
  for(int a=0;a<4;a++) for(int b=0;b<3;b++){ KK=Ks[a]; NN=Ns[b]; one(); }
  mark("--- M 무관성 재확인 ---");
  MM=512; KK=1024; NN=1152; one();
  MM=512; KK=2048; NN=1152; one();
  MM=256; KK=1024; NN=2304; one();
  mark("=== BLK84_DONE ===");
  return 0; }
