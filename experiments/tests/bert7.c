// bert7.c — E416: (16,4)의 단계 이득이 층으로 옮겨가는가. 사전 등록: PREREG_E416.txt
// BERT-base 층, **seq=256**. 스케줄 A = 전 단계 (8,8), B = FFN1만 (16,4).
// FFN1 [768x3072]은 N/K=4로 (16,4)이 이기는 구간이고, FFN2 [3072x768]은 N/K=0.25로
// 지는 구간이라 (8,8) 그대로 둔다.
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
...
static int run_sched(int blkset,int qkv_mode,int mq,int mo,int m1,int m2,double *t){
  ...
  /* E416: blkset 0 = 전 단계 (8,8);  1 = FFN1만 (16,4) */
  const int QI=8, QJ=8;
  const int F1I=(blkset?16:8), F1J=(blkset?4:8);
...
int main(int argc, char **argv){
  g=fopen(argc>1?argv[1]:"/mnt2/tmp/bert7.log","w");
  ...
  mark("=== E416: BERT-base 층 seq=256 — FFN1에 (16,4) (3가속기+mem2) ===");
  ...
  struct { const char *nm; int bs,qm,mq,mo,m1,m2; } S[] = {
    { "A 전부 (8,8)        m=2", 0,1,2,2,2,2 },
    { "B FFN1만 (16,4)     m=2", 1,1,2,2,2,2 },
    { "A 전부 (8,8)        m=3", 0,1,3,3,3,3 },
    { "B FFN1만 (16,4)     m=3", 1,1,3,3,3,3 },
  };
  double base=0;
