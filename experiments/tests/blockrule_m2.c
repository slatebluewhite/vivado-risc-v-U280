// blockrule.c — E304: 완성된 튜닝 규칙(블록+청크+전치)의 표본 밖 검정.
// 사전 등록: experiments/model/PREREG_E304.txt
//
// E302·E303의 여덟 형상을 (4,4)·(4,8)로만 쟀다. 규칙의 1순위인 (8,4)가 그 데이터에 없다.
// 여기서 (8,4)·(4,8)·(4,4) x 청크 2·4·8을 다 훑고, 규칙의 선택이 최적 대비 몇 %인지 센다.
// 블록 조항(E278)은 K >= 1536 형상들에서 세워졌는데 여기는 전부 K <= 1280이다.
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <stdarg.h>
...
int main(int argc,char**argv){
  g=fopen(argc>1?argv[1]:"/mnt2/tmp/br62.log","w");
  ...
  mark("=== [2가속기 62.5MHz] E323 완성 규칙의 표본 밖 검정: (8,4) 포함 전수 훑기 (M=128, best-of-5) ===");
  mark("규칙: 블록 (8,4)+전치 | 청크 K·N>=1.4M ? 4 : 2");
  mark("%11s %6s %4s %5s | %9s %9s %9s | %11s | %s",
       "형상","블록","Kc","청크","m=1","m=2","m=3","η(2) η(3)","정확성");
  MM=128;
  struct { int K,N; } SH[]={{768,1024},{896,1024},{1024,1024},{768,1536},
                            {1024,1280},{1152,1152},{1280,1280},{1024,2048}};
  int BLI[]={8,4,4}, BLJ[]={4,8,4};
  int CH[]={2,4,8};
  for(unsigned h=0;h<8;h++){ KK=SH[h].K; NN=SH[h].N;
    int Ktil=KK/16;
    mark("--- [%dx%d] K·N=%.2fM 규칙: 청크%d ---",KK,NN,(double)KK*NN/1e6,
         ((double)KK*NN>=1.4e6)?4:2);
