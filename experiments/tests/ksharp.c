// ksharp.c — a'(K)의 5~6배 도약은 문턱인가 곡선인가 (E251).
//
// a'(1024)=0.21, a'(2048)=1.39 — K가 2배인데 항이 6배다. E243이 L2 라이브 풋프린트로
// 설명하려다 E244에서 개입으로 기각된 그 특징이고, 모델에 남은 가장 큰 미설명이다.
// K를 256 간격으로 훑어 **모양**을 본다. 문턱이면 그 위치가 자원 크기를 지목하고,
// 완만하면 곡선으로 확정된다.
// N은 nj가 6의 배수(576->6, 1152->12)라 불균형 1.0, 꼬리 블록 없음.
//
...
int main(int argc,char**argv){
  ...
  mark("=== a'(K)의 모양: K를 256 간격으로 (3i8f50 128비트, M=128, (4,6), best-of-3) ===");
  mark("기지: a'(768)=0.22  a'(1024)=0.21  a'(2048)=1.39  a'(3072)=1.73");
  mark("%11s %4s %8s | %9s %9s %9s | %11s | %s",
       "형상","nj","루프라인","m=1","m=2","m=3","η(2) η(3)","정확성");
  int Ks[]={768,1024,1280,1536,1792,2048,2560,3072}, Ns[]={576,1152};
  for(int a=0;a<8;a++) for(int b=0;b<2;b++){
