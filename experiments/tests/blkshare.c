...
static int BLK = 4;           // I*J <= 32 (누산기 절반) — E179의 안전 규칙
...
int main(int argc, char **argv){
  ...
  // E181 예측: 블록 크기(=이동 바이트)는 가속기 하나일 때 거의 무의미하고
  // (DMA가 연산에 가려짐) 셋일 때는 공유 대역폭을 나눠 쓰므로 바이트에 비례해야 한다.
  mark("=== 블록 크기 x 가속기 수 (3i8f50, best-of-5) ===");
  mark("%5s %4s %4s %9s | %8s | %9s | %8s | %s",
       "크기","블록","개수","바이트","시간ms","예측ms","B/cycle","정확성");
  int Ns[]={192};
  for(unsigned i=0;i<sizeof(Ns)/sizeof(Ns[0]);i++){
    SZ=Ns[i]; fill();
    int blks[]={2,4};
    for(unsigned q=0;q<2;q++){ BLK=blks[q]; double b1=bytes_one();
    for(NACC=1; NACC<=3; NACC+=2){
      ...
      double tot = b1*NACC;
      /* 예측: max(연산 바닥, 바이트 / (15.0/NACC)) — 연산 바닥은 192³에서 실측 1.12배 */
      double comp = 1.12*(double)SZ*SZ*SZ/256.0/FREQ;
      double bw   = b1/(15.0/NACC)/FREQ;
      double pred = comp>bw ? comp : bw;
      mark("%4d³ %4d %4d %9.0f | %8.3f | %9.3f | %8.2f | %s",
           SZ, BLK, NACC, b1, best*1e3, pred*1e3, tot/(best*FREQ), worst?"FAIL":"PASS");
    } }
  }
  mark("=== ROOFLINE2_DONE ===");
  return 0; }
