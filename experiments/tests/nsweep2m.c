// nsweep2.c — E311: N=1024가 나쁜 점이라면 BERT-large를 패딩으로 구할 수 있는가.
// E310: K=768에서 N=1024만 산포 16.9%(이웃 1.7%)이고 η3도 최소 대비 +16.4%다.
// 그런데 1024는 BERT-large의 hidden 크기다 — QKV/출력이 [1024x1024]로 그 점에 앉아 있다.
// 여기서는 K=1024 고정, 규칙이 고르는 블록 (8,4)로 N을 감싼다.
// 판정: N=1088의 절대 시간이 N=1024보다 짧으면(일을 6.25% 더 하는데도) 패딩이 순이득이다.
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <stdarg.h>
...
static void fill(void){
  ...
  memset(B,0,(size_t)KK*NN);   /* E312d: nstride와 유일하게 다른 부분 */
...
int main(int argc,char**argv){
  g=fopen(argc>1?argv[1]:"/mnt2/tmp/nsweep2m.log","w");
  ...
  mark("=== E312d [fill에 B memset 추가] BERT-large 단계 [1024xN] 패딩 검정 (블록 (8,4), M=128) ===");
  mark("%11s %6s %4s %5s | %9s %9s %9s | %11s | %s",
       "형상","블록","Kc","청크","m=1","m=2","m=3","η(2) η(3)","정확성");
  MM=128; KK=1024; BI=8; BJ=4;
  int NS[]={960,1024,1088,1152,1216};
  for(unsigned h=0;h<5;h++){ NN=NS[h];
    if((NN/16)%BJ) continue;
    KCv=32; one();          /* 청크 2 — 규칙의 선택 */
    KCv=16; one(); }        /* 청크 4 — 대조 */
  mark("=== NSWEEP2M_DONE ===");
  return 0; }
