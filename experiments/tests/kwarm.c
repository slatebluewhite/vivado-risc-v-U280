...
static int WARM=0;   /* E390: 1이면 타이머 밖에서 A를 훑는다 */
static volatile long warm_sink;
/* E390: 타이머 밖에서 A를 훑어 캐시에 올린다. 층 안에서 A가 앞 단계의 출력이라
   상주하는 상황을 흉내낸다. */
static void warm_A(void){
  long s=0; const int8_t *p=A;
  for(size_t i=0;i<(size_t)MM*KK;i+=64) s+=p[i];
  warm_sink=s; }
...
int main(int argc,char**argv){
  ...
  mark("=== E390 (WARM=%d):  (8,4) 대 (8,8) 교차 탐색 (N=%d, m=3) ===", WARM, NN_);
