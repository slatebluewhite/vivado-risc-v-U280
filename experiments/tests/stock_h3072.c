// stock_h3072.c — 기준선(큰 모델). §5b의 천장이 내 런타임 라이브러리 탓인지
// 하드웨어 성질인지 가른다: 스톡도 같은 천장에 걸리면 하드웨어다.
// (원본) stock_bert.c — 기준선: 스톡 Gemmini 라이브러리(tiled_matmul_auto)를 가속기 하나로.
//
...
int main(int argc,char**argv){
  g=fopen(argc>1?argv[1]:"/mnt2/tmp/stock_h3072.log","w");
  ...
  mark("hidden 3072 층의 matmul만: QKV(3회) + 출력 + FFN1 + FFN2");
  mark("%-10s %10s","단계","ms");
  double best=1e30;
  for(int r=0;r<3;r++){
    double t0=now();
