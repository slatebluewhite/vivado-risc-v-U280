// blk44.c — 블록 간 곡선이 배율만 다른가 (E264).
// 사전 등록: experiments/model/PREREG_E264.txt
// (4,4)에서 6점을 재고, 그중 2점으로 α·β를 보정해 나머지 4점을 예측한다.
// 사전 등록: experiments/model/PREREG_E263.txt
//
...
int main(int argc,char**argv){
  ...
  mark("=== 블록 (4,4): 배율 가설 검정 (3i8f50 현재 상태, M=128, Kc=16, best-of-5) ===");
  mark("(512,1152)와 (512,4608)로 α·β를 보정하고 나머지 4점을 예측한다");
  mark("%4s %11s %5s %9s | %9s %9s %9s | %17s | %s",
       "M","형상","nj","루프라인","m=1","m=2","m=3","η(1) η(2) η(3)","정확성");
  int Ks[]={512,2048}, Ns[]={1152,2304,4608};
  MM=128;
  for(int a=0;a<2;a++) for(int b=0;b<3;b++){ KK=Ks[a]; NN=Ns[b]; one(); }
  mark("=== BLK84_DONE ===");
  return 0; }
