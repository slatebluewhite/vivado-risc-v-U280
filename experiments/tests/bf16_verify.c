// bf16_verify.c — BF16 연산 정확성 자체 검증 (E52 후속)
//
...
static elem_t C[N][N] row_align(1);
...
int main() {
  ...
  for (size_t i = 0; i < N; i++)
    for (size_t j = 0; j < N; j++) {
      ...
      double got = (double)bf16_to_f32(C[i][j]);
  ...
  if (convfail) { printf("BF16_VERIFY INVALID\n"); return 2; }
  if (bad == 0) { printf("BF16_VERIFY PASS\n"); return 0; }
  printf("BF16_VERIFY FAIL\n");
  return 1;
}
