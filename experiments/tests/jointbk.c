...
int main(int argc,char**argv){
  ...
  mark("=== (I,J) x Kc 공동 훑기 (권장 구성 256비트, M=128, K=3072, N=1152, best-of-5) ===");
  mark("제약: Kc·(I+J) <= 스크래치패드. FAIL이 한계를 알려 준다.");
  mark("%5s %11s %4s %7s %9s | %9s %9s %9s | %11s | %s",
       "Kc","형상","청크","루프라인","","m=1","m=2","m=3","η(2) η(3)","정확성");
  int SI[]={8,4,4,2,8}, SJ[]={4,4,6,4,2};
  int Kcs[]={16,32,48,64,96};
  MM=128; KK=3072; NN=1152;
  for(int b=0;b<5;b++){ BI=SI[b]; BJ=SJ[b];
    for(int c=0;c<5;c++){ KCv=Kcs[c]; one(); } }
  mark("=== BLK84_DONE ===");
  return 0; }
