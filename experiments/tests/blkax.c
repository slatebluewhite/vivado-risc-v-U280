// blkax.c — 블록 모양 축: (4,6)에서 적합한 곡선이 다른 (I,J)로 어떻게 옮겨가는가 (E260).
// 사전 등록: experiments/model/PREREG_E260.txt
//   H1 바이트 비례 (자유 파라미터 0) / H0 무변화.  바이트비는 0.90~1.59로 벌어진다.
//
...
int main(int argc,char**argv){
  ...
  mark("=== 블록 모양 축 (3i8f50 현재 상태, M=128, Kc=16, best-of-5) ===");
  mark("모든 (I,J)에서 꼬리 없음, m=3 불균형 정확히 1.0");
  mark("%11s %4s %8s | %9s %9s %9s | %11s | %s",
       "형상","nj","루프라인","m=1","m=2","m=3","η(2) η(3)","정확성");
  struct { int K,N; } S[]={{1024,1152},{2048,1152},{1024,2304},{2048,2304}};
  int SI[]={4,8,4,2}, SJ[]={6,4,4,6};
  for(unsigned si=0; si<4; si++){
   ...
   for(int bi=0; bi<4; bi++){
    ...
    for(int m=1;m<=3;m++){
      ...
      t[m-1]=bt; }
    mark("[%4dx%4d] (%d,%d) nj=%2d | %9.3f %9.3f %9.3f | η %5.2f %5.2f %5.2f | %s",
         KK,NN,BI,BJ,(NN/16)/BJ,t[0]*1e3,t[1]*1e3,t[2]*1e3,
