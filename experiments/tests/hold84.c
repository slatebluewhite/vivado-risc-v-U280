// hold84.c — 블록 (8,4) 모델의 사전 등록 홀드아웃 (E265).
// 사전 등록: experiments/model/PREREG_E265.txt
// K∈{1536,2560}, N∈{1920,3840} 전부 훈련 18점에 없다.
//
...
static void one(void){
  ...
  for(int m=1;m<=3;m++){
    ...
    t[m-1]=b; }
  mark("%4d [%4dx%4d] nj=%2d %9.3f | %9.3f %9.3f %9.3f | %5.2f %5.2f %5.2f | %s",
       MM,KK,NN,(NN/16)/BJ,roof*1e3,t[0]*1e3,t[1]*1e3,t[2]*1e3,
...
int main(int argc,char**argv){
  ...
  mark("=== (8,4) 모델 홀드아웃 (3i8f50 현재 상태, M=128, Kc=16, best-of-5) ===");
  mark("예측 m=3: 18.75 / 38.75 / 40.16 / 81.37 / 22.56 / 24.08 ms");
  mark("%4s %11s %5s %9s | %9s %9s %9s | %17s | %s",
       "M","형상","nj","루프라인","m=1","m=2","m=3","η(1) η(2) η(3)","정확성");
  struct { int K,N; } S[]={{1536,1920},{1536,3840},{2560,1920},{2560,3840},{1536,2304},{2560,1152}};
  MM=128;
  for(unsigned si=0; si<6; si++){ KK=S[si].K; NN=S[si].N; one(); }
  mark("=== BLK84_DONE ===");
  return 0; }
