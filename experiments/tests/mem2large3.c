...
static int run_sched(int blkset,int qkv_mode,int mq,int mo,int m1,int m2,double *t){
  /* E366: 0=A(옛 규칙) 1=B(전부 (8,8)) 2=C(FFN2만 (4,8)) 3=D(FFN2만 (8,8)) */
  const int QJ = (blkset==1) ? 8 : 4, F1J = (blkset==1) ? 8 : 4;
  int F2I,F2J;
  switch(blkset){ case 0: F2I=4;F2J=4; break; case 2: F2I=4;F2J=8; break;
                  default: F2I=8;F2J=8; }
  int bad=0; double t0;
  setblk(8,QJ,32);
...
int main(int argc, char **argv){
  ...
  struct { const char *nm; int bs,qm,mq,mo,m1,m2; } S[] = {
    ...
    { "C FFN2 (4,8) m=1",        2,1,1,1,1,1 },
    { "A 옛규칙  m=2",           0,1,2,2,2,2 },
    { "B 전부(8,8) m=2",         1,1,2,2,2,2 },
    { "C FFN2 (4,8) m=2",        2,1,2,2,2,2 },
    { "D FFN2 (8,8) m=2",        3,1,2,2,2,2 },
    { "A 옛규칙 m=3", 0,1,3,3,3,3 },
    { "B 전부(8,8) m=3", 1,1,3,3,3,3 },
    { "C FFN2 (4,8) m=3", 2,1,3,3,3,3 },
    { "D FFN2 (8,8) m=3", 3,1,3,3,3,3 },
  };
  double base=0;
