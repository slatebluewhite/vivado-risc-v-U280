// mid5.c — E407: h1536 층을 새 K*N 임계(E406)로 다시 잰다.
//
// [1536x1536] (QKV·출력)은 K*N = 2.25 MB로 새 임계 2.6 MB **아래**이므로 목표가 4청크에서
// 2청크로 바뀐다: Kc 24 -> 48. E405가 단독 형상에서 11.4% 차이를 봤다(7.552 -> 6.782).
// FFN1 [1536x6144]은 9 MB로 목표 4 그대로(Kc=24), FFN2도 Kc=64 그대로다.
// QKV+출력은 층의 약 24%이므로 2~3%가 예상되지만, E325 이후 단계 합을 층으로 믿지 않는다.
//
// 블록은 권장인 (8,8) 고정. 스케줄 A=Kc24(옛) B=Kc48(새) C=Kc32(중간, 3청크), m=2·m=3.
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <stdarg.h>
...
static int run_sched(int blkset,int qkv_mode,int mq,int mo,int m1,int m2,double *t){
  ...
  /* E407: blkset = QKV/출력의 Kc만 바꾼다. 블록은 (8,8) 고정. */
  static const int QKC[3]={24,48,32};
  const int QI=8, QJ=8;
  const int F1I=8, F1J=8;
  const int F2I=8, F2J=8, F2KC=64;
  const int QKc=QKC[blkset];
  int bad=0; double t0;
  setblk(QI,QJ,QKc);
...
int main(int argc, char **argv){
  ...
  mark("=== E407: h1536 층 — 새 K*N 임계의 Kc 확인 (블록 (8,8) 고정) ===");
  ...
  struct { const char *nm; int bs,qm,mq,mo,m1,m2; } S[] = {
    { "A QKV/출력 Kc24 (옛,4청크) m=2", 0,1,2,2,2,2 },
    { "B QKV/출력 Kc48 (새,2청크) m=2", 1,1,2,2,2,2 },
    { "C QKV/출력 Kc32 (3청크)    m=2", 2,1,2,2,2,2 },
    { "A QKV/출력 Kc24 (옛,4청크) m=3", 0,1,3,3,3,3 },
    { "B QKV/출력 Kc48 (새,2청크) m=3", 1,1,3,3,3,3 },
    { "C QKV/출력 Kc32 (3청크)    m=3", 2,1,3,3,3,3 },
  };
  double base=0;
