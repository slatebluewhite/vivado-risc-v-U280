...
int main(int argc, char **argv){
  ...
  mark("=== B_dram(ws) 곡선 확장 (mvin 4타일 폭, 가속기당 %d MB 이동) ===", TOTB/1048576);
  for(int a=0;a<3;a++) mark("가속기%d 왕복 검증: 불일치 %d", a, verify(a));
  mark("%9s %9s %5s | %9s | %9s", "총작업셋KB","가속기당KB","개수","시간ms","B/cycle");
  int F[]={2048, 4096, 8192, 12288, 16384, 24576, 32768};   /* 행 수 */
  for(unsigned q=0;q<sizeof(F)/sizeof(F[0]);q++){
    FOOT=F[q];
    for(NACC=1; NACC<=3; NACC++){
      ...
      mark("%9d %9d %5d | %9.3f | %9.2f",
           FOOT*W*NACC/1024, FOOT*W/1024, NACC, best*1e3, by/(best*FREQ));
    } }
  mark("=== BWEXT_DONE ===");
  return 0; }
