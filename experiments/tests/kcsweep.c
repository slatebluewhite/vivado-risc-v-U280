// kcsweep.c — a_rest(K)는 K 자체인가 K 분할 구조인가 (E269).
...
static void one(void){
  ...
  for(int m=1;m<=3;m++){
    ...
    t[m-1]=b; }
  mark("%5d [%4dx%4d] %4d %9.3f | %9.3f %9.3f %9.3f | %5.2f %5.2f %5.2f | %s",
       KCv,KK,NN,(KK/16)/KCv,roof*1e3,t[0]*1e3,t[1]*1e3,t[2]*1e3,
...
int main(int argc,char**argv){
  ...
  mark("=== Kc 훑기 (권장 구성 256비트, M=128, 블록 (8,4), N=1152, best-of-5) ===");
  mark("H_chunk: Kc를 키우면 청크가 줄어 a_rest도 줄어야 한다 / H_K: 안 변한다");
  mark("%5s %11s %4s %7s %9s | %9s %9s %9s | %11s | %s",
       "Kc","형상","청크","루프라인","","m=1","m=2","m=3","η(2) η(3)","정확성");
  int Ks[]={1024,3072}, Kcs[]={8,16,32,64};
  MM=128; NN=1152;
  for(int a=0;a<2;a++){ KK=Ks[a];
    for(int c=0;c<4;c++){ KCv=Kcs[c]; one(); } }
  mark("=== BLK84_DONE ===");
  return 0; }
